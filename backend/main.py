...
class UpdateAppointmentStatusRequest(BaseModel):
    status: str # e.g. "Waiting", "In-Progress", "Completed"
...
def register_user(req: RegisterUserRequest):
    uid = database.create_user(nickname=req.nickname, identity_mode=req.identity_mode, age_group=req.age_group)
...
def register_therapist(req: RegisterTherapistRequest):
    ...
    try: return {"therapist_id": database.create_therapist(req.name, req.email, pw, req.specialization)}
...
def book_appointment(req: BookAppointmentRequest):
    app_id = database.create_appointment(req.user_id, req.therapist_id, req.scheduled_time, req.request_type)
    return {"appointment_id": app_id, "status": "Pending Confirmation", "request_type": req.request_type}
...
#   2. risk_scoring_node       — ML + regex combined risk score
#   3. [risk_router]           — Conditional edge: crisis / panic / normal
#   4. sentiment_enrichment    — Gemini sentiment (normal path only)
#   5. build_context_node      — Assembles LLM prompt context
#   6. response_node           — Gemini LLM therapeutic response
#   7. logging_node            — Persists everything to DB
#
# The user sees NONE of this. They just get exceptional responses.
# ==============================================================
...
def chat_endpoint(request: ChatRequest):
    try:
        ...
        result = mindbridge_graph.invoke({
            ...
            "age_group":           age_group,
            "user_text":           user_text,
            # Fields populated by graph nodes (initialised empty)
            "ml_crisis":           {},
            "ml_emotion":          {},
            "ml_distortions":      {},
            "regex_risk":          {},
        ...
        # Strip out the programmatic AI metadata from the user-facing response
        ...
        if "ACTION: BOOK_SESSION" in display_text:
            ...
            # Programmatically book the session (AI-Initiated)
            from datetime import timedelta
            scheduled_time = (datetime.now() + timedelta(hours=2)).isoformat()
            database.create_appointment(request.user_id, 't_dr_smith', scheduled_time, "AI-Initiated")
...
# ══════════════════════════════════════════════════════════
# AI: Full Session Analysis (real-time)
# ══════════════════════════════════════════════════════════
...
def get_ai_insights(user_id: str):
    ...
    return api_services.generate_ai_insights(profile, stats.get("mood_history", []), stats.get("risk_history", []))
...
def auto_generate_treatment_plan(user_id: str):
    ...
    if not profile:
        raise HTTPException(404, "Profile not found")
    # Fetch recent messages across all their sessions to give the AI context
...
def session_summary(req: SummaryRequest):
    return api_services.generate_session_summary([{"role": m.role, "content": m.content} for m in req.messages])
...
def submit_feedback(req: FeedbackRequest):
    database.submit_feedback(req.user_id, req.session_id, req.session_type, req.rating, req.comments)
    return {"status": "ok"}
